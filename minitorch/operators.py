...
def zipWith(fn):
    ...
    """
    # TODO: Implement for Task 0.3.
    def new_fun(list1, list2):
        new_list = []
        for counter, value in enumerate(list1):
            new_list.append(fn(list1[counter], list2[counter]))
        return new_list
    return new_fun
...
def reduce(fn, start):
    ...
    # TODO: Implement for Task 0.3.
    def new_fun(list1):
        n_list = list(list1).copy()
        if len(n_list) == 0:
            return start
        current_value = n_list.pop()
        return fn(current_value, new_fun(n_list))
    return new_fun
