...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        return [(inp, d_input) for inp, d_input in zip(inputs, d_output) if not is_constant(inp)]
...
def backpropagate(variable, deriv):
    ...
    var_der = {v.unique_id: 0 for v in order}
    var_der[variable.unique_id] = deriv
    #
    for var in order:
        if var.is_leaf():
            var.accumulate_derivative(var_der[var.unique_id])
        else:
            for var_,deriv_ in var.history.backprop_step[var_der[var.unique_id]]:
                if var_.unique_id in var_der:
                    var_der[var_.unique_id] += deriv_
                else:
                    var_der[var_.unique_id] = deriv_
                    '''
